...
public class TrainingBookWorkbook {

	protected WritableWorkbook buildExcelDocument(WritableWorkbook workbook, Training training, List<Student2> studentList, List<CalendarManage> calendar, TrainingBook trainingBook,
			Map<Integer, Map<String, String>> trainingBookRepo, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 헤더 컬럼 지정
		String title = String.format("[%s-%s] %s / 강의기간 : %s ~ %s, 강의시간 : %s ~ %s, 총 인원 : %s", training.getGroup_name(), training.getCategory_name(), training.getTraining_name(), training.getStart_date(), training.getEnd_date(), training.getStart_time(), training.getEnd_time(), studentList.size());
		...
		for ( int i = 0; i < studentList.size(); i ++ ) {
			Student2 oneStudent = studentList.get(i);
			...
			// 다시 초기화
	 		startDayCul = 2;
			// ${trainingBookRepo[oneStudent.student_idx][plan_date] eq '1'}
			for ( CalendarManage c : calendar ) {
				if ( StringUtils.isNotEmpty(c.getSun()) ) {
					String key = String.format("%s-%s", c.getPlan_date(), c.getSun().length() == 1 ? "0" + c.getSun():c.getSun());
					String status = null;
					try {
						status = trainingBookRepo.get(oneStudent.getStudent_idx()).get(key);
					} catch ( Exception e ) {
					}

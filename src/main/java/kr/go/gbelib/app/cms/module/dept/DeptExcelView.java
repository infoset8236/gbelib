...
public class DeptExcelView extends AbstractJExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, WritableWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		workbook.getSheet(0).setColumnView(7,  40);

		// 헤더 컬럼 지정
		workbook.getSheet(0).addCell(new Label(0, 0, "기관코드(PK)", format));
		workbook.getSheet(0).addCell(new Label(1, 0, "기관명", format));
		workbook.getSheet(0).addCell(new Label(2, 0, "그룹명", format));
		workbook.getSheet(0).addCell(new Label(3, 0, "우편번호", format));
		workbook.getSheet(0).addCell(new Label(4, 0, "우편주소", format));
		workbook.getSheet(0).addCell(new Label(5, 0, "사용여부 Y/N", format));
		workbook.getSheet(0).addCell(new Label(6, 0, "담당자명", format));
		workbook.getSheet(0).addCell(new Label(7, 0, "담당자폰번호", format));
	}
}

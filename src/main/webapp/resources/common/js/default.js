...
$(function(){
	...
	$('table tr').each(function(){
		...
		$(this).children('th').each(function(e){
			$(this).addClass('th'+(e+1));
		});
		$(this).children('td').each(function(e){
			$(this).addClass('td'+(e+1));
		});
	});
	$('table col').each(function(e){
		$(this).addClass('col'+(e+1));
	});
	...
	$('div.bbs-comment-textarea textarea').on('focus focusin',function(){
		$(this).parent().addClass('current');
		$(this).parent().parent().find('.checkbox').show();
	});
	$('div.bbs-comment-textarea textarea').on('blur focusout',function(){
	...
	$('ul.lt_photo li').each(function(e){
		$(this).addClass('lt-gall'+(e+1));
	});
	...
	/*
	 * $('.faq-wrap .faq_q a').on('click',function(e){ e.preventDefault();
	 * if(!($(this).parents('li').hasClass('active'))){ $('.faq-wrap
	 * li').removeClass('active'); $(this).parents('li').addClass('active'); }
	 * });
	 */
	...
	if($('div.Gnb.notype').length > 0){ // 예천
		...
		$('div.Gnb ul.gnb-menu li.List, div.Gnb div.mask').on('mouseover focusin', function(){
			$('div.Gnb ul.gnb-menu li.List').removeClass('active');
			$(this).addClass('active');
			$('div.Gnb').addClass('on');
			$('div.mask').show();
		});
		$('div.Gnb ul.gnb-menu li.List ul, div.Gnb div.mask').on('mouseout focusout', function(){
	...
	var _str1 = '<i class="fa fa-minus"></i>';
	var _str2 = '<i class="fa fa-plus"></i>';
	var _str3 = 'i';
	 ...
	 * if($(this).hasClass('active')){ $(this).children('a').append('<i
	 * class="fa fa-minus"></i>'); }else{ $(this).children('a').append('<i
	 * class="fa fa-plus"></i>'); }
	 * $(this).children('a').on('click',function(){
	 * if($(this).parent().hasClass('active')){ $(this).children('i').remove();
	 * $(this).append('<i class="fa fa-plus"></i>');
	 * 
	 * $('div.lnb > ul > li > ul > li > ul').slideUp(80); $('div.lnb > ul > li >
	 * ul > li').removeClass('active'); $(this).parent().removeClass('active');
	 * }else{ $(this).parent().parent().children('li').each(function(){
	 * if($(this).children('a').children('i').hasClass('fa-minus')) {
	 * $(this).children('a').children('i').remove();
	 * $(this).children('a').append('<i class="fa fa-plus"></i>'); } });
	 * $(this).children('i').remove(); $(this).append('<i class="fa fa-minus"></i>');
	 * 
	 * $('div.lnb > ul > li > ul > li > ul').slideUp(80); $('div.lnb > ul > li >
	...
	$('div.mmode > a').on('click', function() { 
		if ( mmenuFlag ) {
			try {
				 ...
				 * $(".mmenu_close").click(function(){ API.close(); });
				 */
			} catch (e) {
			...
			mmenuFlag = false;
			$('li.active').each(function(i,v) {
				$('nav li.'+$(this).attr('id')).addClass('mm-opened').parent().parent().parent().addClass('mm-opened');
			});			
		}
		$('nav#menu li').show();
	} );
	...
	// 팝업존2 - 영일용
	if ($('div.popupzonenew2 ul').length > 0) {
		$('div.popupzonenew2 ul').bxSlider({
			mode:'fade',
			auto: true,
			autoHover: true,
			speed: 500,
	...
	if (bannerLength2 > 0) {
		if(banner2.hasClass('display3')){
			var ggg = 3;
		}else{
			var ggg = 2;
		}
		bannerSlider2 = $('div.banner-wrap.type2 ul.banner-roll').bxSlider({
			mode:'vertical',
			slideWidth:150,
			speed:500,
			moveSlides:1,
	...
	if (bannerLength3 > 0) {
		bannerSlider3 = $('div.banner-wrap.type1 ul.banner-rolls').bxSlider({
			slideWidth:187,
			speed:500,
			moveSlides:1,
			maxSlides:8,
	...
	if (bannerLength4 > 0) {
		bannerSlider4 = $('div.banner-wrap.type4 ul.banner-roll').bxSlider({
			slideWidth:187,
			speed:500,
			moveSlides:1,
			maxSlides:8,
	...
	if (bannerLength5 > 0) {
		bannerSlider5 = $('div.banner-wrap.type5 ul.banner-roll').bxSlider({
			slideWidth:187,
			speed:500,
			moveSlides:1,
			maxSlides:8,
	...
	if (bannerLength6 > 0) {
		bannerSlider6 = $('div.banner-wrap.type6 ul.banner-roll').bxSlider({
			slideWidth:187,
			speed:500,
			moveSlides:1,
			maxSlides:8,
	...
	var fsiteLink = $('div#foot_section a.fsite.type1');
	if (fsiteLink.length > 0) {
		
		fsiteLink.on('click', function(e){
			e.stopPropagation();
			$('div#foot_section a.fsite').next().hide();
			if($(this).hasClass('active')){
				$(this).removeClass('active').next().hide();
			}else{
				$('div#foot_section a.fsite').removeClass('active').next().hide;
				$(this).addClass('active').next().show();
			}
	...
	var fsiteLink2 = $('div#foot_section a.fsite.type2');
	if (fsiteLink2.length > 0) {
		fsiteLink2.on('click', function(e){
			e.stopPropagation();
			$('div#foot_section a.fsite').next().hide();
	...
	if(window.location.pathname.match("^/geic/")) {
	if ($('ul.main_img').length > 0) {
		$('ul.main_img').bxSlider({
			mode:'fade',
			auto:true,
			pager:false,
			autoControls:true,
		...
		$('ul.main_img').parent().next().addClass('mainImg');
	}
	} else {
	if ($('ul.main_img').length > 0) {
		$('ul.main_img').bxSlider({
	...
	// 추천도서
	if ($('div.like_book ul').length > 0) {
		$('div.like_book ul').bxSlider({
			auto:true,
			autoControls:true,
			autoHover:true,
			autoReload:true,
	...
	if ($('div.like_book2 ul').length > 0) {
		$('div.like_book2.on ul').bxSlider({
			auto:true,
			autoControls:true,
			autoHover:true,
			autoReload:true,
		...
		$('div.like_book2 h3 a').on('click',function(e){
			e.preventDefault();
		    $('div.like_book2 li, div.like_book2 h3 a').removeClass('active');
		    $(this).addClass('active');
		    var activeTab = $(this).attr('href');
		    $(activeTab).addClass('active');
		});
...
*/
	if ($('div.cultureList ul').length > 0) {
		$('div.cultureList ul').bxSlider({
			auto: true,
			autoHover: true,
			speed: 500,
			pager:false,
	...
	$('body div#wrap li.1Depth > a').on('click', function(e) {
	...
	if (elibSiteLink.length > 0) {
		...
		elibSiteLink.on('click', function(e){
			e.stopPropagation();
			$('div.elib_top a.elibSite.type1').next().hide();
			if($(this).hasClass('active')){
				$(this).removeClass('active').next().hide();
			}else{
				$('div.elib_top a.elibSite.type1').removeClass('active').next().hide;
				$(this).addClass('active').next().show();
			}

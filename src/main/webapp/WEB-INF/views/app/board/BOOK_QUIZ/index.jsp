...
<div class="wrapper-bbs">
	...
	<div class="table-wrap">
		<table class="bbs center" summary="이벤트 게시판">
			...
			<thead>
				<tr>
					...
					<c:choose>
						<c:when test="${authMBA}">
							<th class="">제목 / 추천도서 / 저자 / 전화번호</th>
						</c:when>
						<c:otherwise>
							<th class="">제목 / 추천도서 / 저자</th>
						</c:otherwise>
					</c:choose>
				...
				<tr${i.group_depth > 0?' class="reply"':''}>
					...
					<td class="important left title" style="padding-left:${(i.group_depth > 0 ? (i.group_depth-1)*15 : 0)+10}px;">
						...
						<c:if test="${i.group_depth > 0}">
							...
							<c:choose>
								<c:when test="${authMBA}">
									<span>${i.title} / ${i.imsi_v_7} / ${i.imsi_v_8} / ${i.user_phone}</span>
									${i.secret_yn eq 'Y'?'<i class="fa fa-lock"></i>':''}
								</c:when>
								<c:otherwise>
									<span>${i.title} / ${i.imsi_v_7} / ${i.imsi_v_8}</span>
									${i.secret_yn eq 'Y'?'<i class="fa fa-lock"></i>':''}
								</c:otherwise>
							</c:choose>

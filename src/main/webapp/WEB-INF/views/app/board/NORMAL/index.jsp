...
<c:if test="${board.menu_idx eq '158' and board.homepage_id eq 'h31'}">
	<div class="tabmenu tab2">
		<ul>
			<li><a href="/gbccs/html.do?menu_idx=175">2014년 이후</a></li>
			<li class="active"><a href="/gbccs/board/index.do?menu_idx=158&manage_idx=654">2014년 이전 자료</a></li>
		</ul>
	</div>
</c:if>
		...
		<table class="bbs center" summary="일반 게시판">
			...
			<thead>
				<tr>
					...
					<c:if test="${board.homepage_id ne 'h23' and boardManage.menu_idx eq '68'}">
			...
			<c:forEach var="i" varStatus="status" items="${boardNoticeList}">
				<tr class="notice">
					...
					<td class="mmm1 important num adddate"><fmt:formatDate value="${i.add_date}" pattern="yyyy.MM.dd" /></td>
					<c:if test="${board.homepage_id ne 'h23' and boardManage.menu_idx eq '68'}">

...
$(function(){
	...
	$('#searchBtn').on('click', function (e) {
		if ( $('#homepageId').val() && $('#dateType').val() && $('#searchType').val() ) {
			...
			if (new Date($('input#dateEnd').val()).getTime() >= 1569888000000) {
				...
				$('div#graph1').load('accessGraph.do?' + serializeCustom($('#webpageAccessSearch')), function( response, status, xhr ) {
					initGraph();
				});
	 			$('table#accessTableData').load('accessTable.do?' + serializeCustom($('#webpageAccessSearch')), function( response, status, xhr ) {
	 			});
			}
	...
	$('a#excelDownload').on('click', function(e) {
		if ( $('#homepageId').val() && $('#dateType').val() && $('#searchType').val() ) {
			...
			$('#webpageAccessSearch').attr('action', '/cms/webpageAccess/excelDownload.do');
			$('#webpageAccessSearch').submit();
		}
		e.preventDefault();
	});
	...
	$('a#csvDownload').on('click', function(e) {
		if ( $('#homepageId').val() && $('#dateType').val() && $('#searchType').val() ) {
			$('#homepageName').val($('#homepageId option:selected').text());
			$('#webpageAccessSearch').attr('action', '/cms/webpageAccess/csvDownload.do');
			$('#webpageAccessSearch').submit();
		}
		e.preventDefault();
	});
...
<div class="search">
	<form:form id="webpageAccessSearch" modelAttribute="webpageAccess" action="/cms/webpageAccess/excelDownload.do" method="post" style="display:inline-flex">
		...
		<form:select id="searchType" path="search_type" class="selectmenu-search" style="width:200px">
			<option disabled >기준 선택</option>
			<option value="ALL" selected="selected">웹페이지 카운트</option>
			<option value="BROWSER">브라우저별 웹페이지 카운트</option>
			<option value="OS">OS별 웹페이지 카운트</option>
			<option value="DEVICE">기기별 웹페이지 카운트</option>
		</form:select>
		아이디(대출번호) :
...
<div class="alert">
	<ul>
		...
		<li>웹페이지 카운트 통계 기능은 2019 기능 개선 사업으로 인해 2019.09.30까지만 조회 가능합니다.</li>
	</ul>
</div>

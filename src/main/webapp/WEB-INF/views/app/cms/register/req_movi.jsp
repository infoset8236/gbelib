...
String certDate=day;
	...
	String exVar    = "0000000000000000";                                       // 복호화용 임시필드
		...
		String retUrl   = "32http://gnelib.gne.go.kr/mobile/pcc_V3_popup_seed.jsp";
	...
	while(en.hasMoreElements()){
		reqName = (String)en.nextElement();
		reqValue = request.getParameter(reqName);
		if(params.length() > 0) params.append("|");
		params.append(reqName);
		session.setAttribute(reqName, reqValue);
	}
	session.setAttribute("pccParams", params.toString());
	/*
	session.setAttribute("site_url", request.getParameter("site_url"));
...
reqNum : <%=reqNum%>
<%
	session.setAttribute("reqNum1", reqNum);
		//01. 암호화 모듈 선언
...
<!--
document.domain = 'gne.go.kr';
		var PCC_window;

		function openPCCWindow1(){
			var popName = 'PCCV3Window';
			var PCC_window = window.open('', popName, 'width=430, height=560, resizable=1, scrollbars=no, status=0, titlebar=0, toolbar=0, left=300, top=200' );
			//PCC_window.document.write('<body><iframe width="100%", height="100%" name="PCCV3Window" src="" frameborder="0" allowfullscreen></iframe></body>');
			if(PCC_window == null){
				alert(" ※ 휴대폰 설정에서 브라우저 설정 중 팝업 차단을 해제해 주시기 바랍니다. \n\n※ 윈도우 XP SP2 또는 인터넷 익스플로러 7 사용자일 경우에는 \n    화면 상단에 있는 팝업 차단 알림줄을 클릭하여 팝업을 허용해 주시기 바랍니다. \n\n※ MSN,야후,구글 팝업 차단 툴바가 설치된 경우 팝업허용을 해주시기 바랍니다.");
			}

			document.reqPCCForm.action = 'https://pcc.siren24.com/pcc_V3/jsp/pcc_V3_j10.jsp';
			document.reqPCCForm.target = popName;
			document.reqPCCForm.submit();

			if(navigator.userAgent.indexOf("MSIE 9.0") > -1 && navigator.userAgent.indexOf("Trident/6.0") > -1) {
			//	PCC_window.close();
			}
			return true;
		}
...
<!-- 본인실명확인서비스 요청 form --------------------------->
<form name="reqPCCForm" id="reqPCCForm" method="post" action = "">
...
<a href="#" onclick="openPCCWindow1()">요청</a>
...
<% //if(session.getAttribute("mode") != null && ((String)session.getAttribute("mode")).equals("idSearch")) { %>
<script>openPCCWindow1();</script>
<% //} %>

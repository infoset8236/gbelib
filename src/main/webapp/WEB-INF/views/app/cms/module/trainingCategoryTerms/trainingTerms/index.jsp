...
$(function() {
	...
	$('a#delete-btn').on('click', function(e) {
		if ( confirm('해당 이미지를 삭제 하시겠습니까?') ) {
			...
			$('#trainingTermsListForm').attr("action", 'delete.do');
			
			if(doAjaxPost($('#trainingTermsListForm'))) {
				location.reload();
				$('#trainingTermsListForm').attr("action", 'index.do');
			}	
		}
		e.preventDefault();
	});
...
<form:form id="trainingTermsListForm"  modelAttribute="trainingTerms" action="index.do" >
<form:hidden path="terms_idx"/>

	<div class="infodesk">
		검색 결과 : 총 ${trainingTermsListCount}건
		<div class="button">
			<c:if test="${authC}">
	...
	<table class="type1 center">
		...
		<tbody>
			<c:forEach var="i" varStatus="status" items="${trainingTermsList}">
				<tr>
					<td>${trainingTerms.listRowNum - status.index}</td>
					<td>${i.terms_type_name}</td>
					<td><a href="" id="dialog-view" keyValue="${i.terms_idx}">${i.title}</a></td>
					<td>${i.use_yn eq 'Y' ? '사용' : '미사용'}</td>
					<td><span><fmt:formatDate value="${i.add_date}" pattern="yyyy.MM.dd"/></span></td>
					<td>
						<c:if test="${authU}">
			...
			<c:if test="${trainingTermsListCount eq 0}">
				<tr>

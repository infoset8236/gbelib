...
$(document).ready(function() {
	...
	$('select#cate1').on('change', function(e) {
		var training_code = $('select#cate1').val();
		$.get('getSubcategories.do?training_code=' + training_code, function(data) {
			var cate2 = $('select#cate2').empty();
			$(data.data).sort(function(a, b) {
				return parseInt(a.display_seq) > parseInt(b.display_seq);
			}).each(function(i) {
				cate2.append($('<option>', { value: this.training_code, text: this.code_name }));
			});
		});
	});
	...
	$('a#modify_cate').on('click', function(e) {
		...
		$('input#training_code').val($('select#cate1').val());
	...
	$('a#modify_cate2').on('click', function(e) {
		...
		$('input#editMode').val('MODIFY');
		$('input#parent_id').val($('select#cate1').val());
		$('input#training_code').val($('select#cate2').val());
...
function saveList(options) {
	...
	for(var i = 0; i < options.length; i++) {
		data_list.push({ training_code: options[i].value, display_seq: (i + 1) });
	}
...
function addOption(select) {
	return function(data) {
		var cate = data.data;
		$('<option>', { value: cate.training_code, text: cate.code_name }).appendTo(select);
	}
}
...
function modifyOption(select) {
	return function(data) {
		var cate = data.data;
		$(select + ' option:selected').val(cate.training_code);
		...
		<div class="contentsBox">
			<div class="categoryEdit ">
				...
				<select id="cate1" name="cate1" size=2 style="width: 100%; height: 540px;">
					...
					<option value="${i.training_code}">${i.code_name}</option>
					</c:forEach>
				</select>
			</div>
		</div>
...
<form:hidden path="training_code"/>

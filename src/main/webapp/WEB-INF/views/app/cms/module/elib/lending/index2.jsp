...
$(function() {
	...
	$('a#excelDownload').on('click', function(e) {
		if('${fn:length(lendingList)}' > 0) {
			...
			$('#hiddenForm').attr('action', 'excelDownload2.do').submit();
	...
	$('a#csvDownload').on('click', function(e) {
		if('${fn:length(lendingList)}' > 0) {
			...
			$('#hiddenForm_search_sdt').val($('#search_sdt').val());
			$('#hiddenForm_search_edt').val($('#search_edt').val());
			$('#hiddenForm').attr('action', 'csvDownload2.do').submit();
			$('#hiddenForm').attr('action', 'save.do');
		} else {
			alert('해당 내역이 없습니다.');
		}
...
<form:form id="lendingListForm"  modelAttribute="lending" action="index2.do" >
...
<c:if test="${member.admin}">
	...
	<div class="infodesk">
		...
		<form:select path="sortField" class="selectmenu">
			...
			<form:option value="reg_dt">이용일자순</form:option>
		</form:select>
...
>조회된 자료가 없습니다.</td>
	...
	<jsp:include page="/WEB-INF/views/app/cms/common/paging.jsp" flush="false">
		...
		<jsp:param name="pagingUrl" value="index2.do"/>
	</jsp:include>

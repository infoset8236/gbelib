...
$(function() {
	...
	$('a#csvDownload').on('click', function(e) {
		...
		if('${fn:length(elibStatisticsMapList)}' > 0) {
			...
			$('#csv_string').val(csv);
			$('#csv_filename').val('전자도서관_회원소속기준별_카테고리별통계_'+formatDate(new Date())+'.csv');
			$('#csvForm').submit();
		} else {
			alert('해당 내역이 없습니다.');
		}
...
<input type="hidden" id="csv_filename" name="filename" value="전자도서관_회원소속기준별_카테고리별통계.csv">
</form>
...
<form:hidden path="library_code" value="${library_code}"/>
	...
	<div>
		<h4>소속도서관별통계(카테고리별 통계)는 2022년 10월 30일부터 통계가 집계가 됩니다.</h4>
	</div>
	<div class="infodesk">
		<div class="button">

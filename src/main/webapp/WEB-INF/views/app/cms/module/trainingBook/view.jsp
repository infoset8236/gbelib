...
$(document).ready(function() {
	$('div#trainingBookViewArea').dialog({ //모달창 기본 스크립트 선언
		width: '100%',
		height: 700,
		autoOpen: false,
		resizable: false,
		...
		buttons: [
			{
				...
				"class": 'btn btn1',
				click: function() {
					var divToPrint = document.getElementById("trainingBookViewArea");
					   newWin= window.open("");
...
<table class="type2 center" style="">
	...
	<tbody>
		<c:choose>
			<c:when test="${fn:length(trainingBookInfo) > 0 and fn:length(trainingBookInfo[0]) > 1}">
				<c:forEach var="i" varStatus="status" items="${trainingBookInfo}">
				<tr>

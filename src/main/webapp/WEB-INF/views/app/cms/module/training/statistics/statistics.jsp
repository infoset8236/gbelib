...
<table id="statisticsResult" class="chartData center">
	...
	<tbody>
		<c:set var="total_training_count" value="0"/>
		...
		<c:forEach var="i" varStatus="status" items="${statisticsList}">
			<c:set var="total_training_count" value="${total_training_count + i.training_limit_count + i.training_backup_count + i.training_offline_count }"/>
			...
			<tr>
				...
				<td>${i.training_limit_count + i.training_backup_count + i.training_offline_count} / ${i.join_count}</td>
	...
	<tfoot>
		<tr>
			<th>합계</th>
			<td colspan="7">전체 모집 인원 : ${total_training_count}, 전체 참여 인원 : ${total_join_count}, 전체 수료 인원 : ${total_cert_ok_count}, 전체 미수료 인원 : ${total_cert_fail_count}</td>

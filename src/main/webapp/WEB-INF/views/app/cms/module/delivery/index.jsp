...
<script type="text/javascript">
    $(function(){
        $('a#approveBtn').on('click', function(e) {
            e.preventDefault();
            $('input#editMode').val('APPROVE');
            $('input#delivery_idx').val($(this).attr('keyValue1'));
            $('input#delivery_status').val(1);
            $('input#delivery_return_status').val(0);
            if (doAjaxPost($('#delivery'))) {
                location.reload();
            }
        })
        $('a#rejectBtn').on('click', function(e) {
            e.preventDefault();
            $('input#editMode').val('REJECT');
            $('input#delivery_idx').val($(this).attr('keyValue1'));
            $('input#delivery_status').val(2);
            if (doAjaxPost($('#delivery'))) {
                location.reload();
            }
        })
...
<form:form modelAttribute="delivery" action="save.do" method="post" onsubmit="return false;">
    ...
    <form:hidden path="delivery_status"/>
    ...
    <table class="type1 center">
        ...
        <c:forEach var="i" varStatus="status" items="${deliveryList}">
            <tr style="word-break:unset;">
                ...
                <td>
                    <c:choose>
                        <c:when test="${i.delivery_status eq 1}">
                            승인
                        </c:when>
                        <c:when test="${i.delivery_status eq 2}">
                            반려
                        </c:when>
                        <c:otherwise>
                            승인대기
                ...
                <td>
                    <a id="approveBtn" href="" class="btn modify_btn" keyValue1="${i.delivery_idx}">확인</a>
                    <a id="rejectBtn" href="" class="btn cancel_btn" keyValue1="${i.delivery_idx}">반려</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>

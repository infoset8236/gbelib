...
<div class="listBox">
	<div class="sendList">
		...
		<dl class="getNum">
			<dt>&middot; 받는 이메일</dt>
			<dd>
				<input id="rcv_mobileno_temp" name="" type="text" size="15" value="" style="width: 150px;"/>
			</dd>
		</dl>
		<p class="btn_putNum" style="right: 5px;"><a href="#" id="addTelpcs" class="btn"><i class="fa fa-plus"></i>추가</a>
		...
		<!-- 전송목록 -->
		<div class="list">
			<table border="0" sumary="문자전송을 할 사람의 이름과 이메일을 확인할 수 있으며 불필요한 번호는 삭제할 수 있습니다.">
...
$(function() {
		...
		$.ajax({
			...
			success: function(data) {
				...
				for (var i = 0; i < data.length; i++) {
					html += '<tr id="'+ data[i]['address_email'] +'" nm="' + data[i]['address_name'] + '">';
					var disabled = '';
					if (data[i]['address_email'] == null) {
						var disabled = 'disabled="disabled"';
					}
					html += '<td><input type="checkbox" '+disabled+' value="'+data[i]['address_email']+'"></td>';
					html += '<td class="nickname">'+ data[i]['address_name'] +'</td>';
					if (data[i]['address_email'] == null) {
						html += '<td><span></span>';
					} else {
						html += '<td><span>'+ data[i]['address_email'] +'</span>';
					}
					html += '</tr>';
				}
	...
	$('a#addTelpcs').on('click', function(e) {
		...
		if ($('input#rcv_mobileno_temp').val() == '') {
			alert('이메일을 입력해주세요.'); $('input#rcv_mobileno_temp').focus(); return false;
		}
// 		var pattern = /^01[\d]{1}-?[\d]{3,4}-?[\d]{4}$/;
		var pattern = /^[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_.]?[0-9a-zA-Z])*.[a-zA-Z]{2,10}$/i;
		if (!pattern.test($('input#rcv_mobileno_temp').val())) {
			alert('이메일형식이 올바르지 않습니다.'); $('input#rcv_mobileno_temp').focus(); return false;
		}
		duple = false;
		$('tbody#sendList tr').each(function(i, el) {
			if ($(el).attr('id') == $('input#rcv_mobileno_temp').val()) {
				alert('이미 추가된 주소입니다.'); duple = true; return false;
			}
		});
		if (duple) {
			return false;
...
// 		var pattern = /^01[\d]{1}-?[\d]{3,4}-?[\d]{4}$/;
	...
	$('a#chooseDelete').on('click', function(e) {
		...
		if ($('tbody#sendList input[type=checkbox]:checked').length < 1) {
			alert('선택된 주소가 없습니다.'); return false;
		}
...
// 		$('input#rcv_name_ar').val('');
	...
	$('a#arrowLeft').on('click', function(e) {
		...
		if ($('tbody#contactList input[type=checkbox]:checked').length < 1) {
			alert('선택된 주소가 없습니다.\n\n우측 주소록에서 전송하실 주소를 선택해주세요.'); return false;
		}
...
// 		var pattern = /^01[\d]{1}-?[\d]{3,4}-?[\d]{4}$/;
		...
		if ($(data).length == 0) {
			alert('중복된 주소는 추가할 수 없습니다.'); return false;
		} else if ($(data).length != originalLength) {
			alert('중복주소를 제외한 나머지 주소만 추가되었습니다.');
		}
		...
		var option = {
				...
				success: function(response) {
					...
					if(response.valid) {
						...
						for (var i = 0; i < data.length; i++) {
							var phone = data[i]['address_email'];

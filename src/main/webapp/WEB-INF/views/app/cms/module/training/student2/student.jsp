...
$(function(){
	...
	$('a#dialog-add').on('click', function(e) {
		...
		if ( '${student.training_idx}' == 0 ){
			alert('선택된 연수가 없습니다.');
			return false;
		}

		else{
			$('#dialog-1').load('edit.do?editMode=ADD&homepage_id=${student.homepage_id}&group_idx=${student.group_idx}&category_idx=${student.category_idx}&training_idx=${student.training_idx}', function( response, status, xhr ) {
	...
	$('a.dialog-modify').on('click', function(e) {
		$('#dialog-1').load('edit.do?editMode=MODIFY&homepage_id=${student.homepage_id}&group_idx=${student.group_idx}&category_idx=${student.category_idx}&training_idx=${student.training_idx}&student_idx=' + $(this).attr('keyValue1'), function( response, status, xhr ) {
			$('#dialog-1').dialog('open');
		});
	...
	$('a.delete-btn').on('click', function(e) {
		if ( confirm('해당 수강생을 삭제 하시겠습니까?') ) {
			...
			if(doAjaxPost($('#delForm'))) {
				$('button.training_btn_${student.group_idx}${student.category_idx}${student.training_idx}').click();
			}
		}
		e.preventDefault();
	});
	...
	$('a.add_blackList').on('click', function(e) {
		$('#dialog-3').load('/cms/module/blackList/edit.do?editMode=ADD&black_type=10&after_click_btn=button.training_btn_${student.group_idx}${student.category_idx}${student.training_idx}&homepage_id=' + $(this).attr('homepage_id') + '&member_id=' + $(this).attr('keyValue')+ '&member_key=' + $(this).attr('keyValue1'), function( response, status, xhr ) {
			$('#dialog-3').dialog({
				width: 600,
				height: 400
			});
	...
	$('input#file').change(function(e) {
		...
		if ( sendFile.length > 0 ) {
			...
			data.append('training_idx', $('#studentListForm #training_idx').val());
	...
	$('#batch-black-add').click(function (event) {
		event.preventDefault();

		var selectedBlackList = $('.student_idx_arr:checked').map(function () {
			return {
				student_idx: $(this).val(),
				member_key: $(this).data('member-key'),
				member_name: $(this).data('member-name')
			};
		}).get();

		console.log(JSON.stringify(selectedBlackList));
		...
		if (confirm('선택된 인원을 블랙리스트에 추가 하시겠습니까?')) {
			$('input[name="blackListBatchArray"]').val(JSON.stringify(selectedBlackList));

			$('form#blackListBatch').attr('action', '/cms/module/blackList/blackListBatchAdd.do');
			if (doAjaxPost($('form#blackListBatch'))) {
				location.reload();
			}
		}
	});

	$('#batch-black-cancel').click(function (event) {
		event.preventDefault();
		...
		if (confirm('선택된 인원을 블랙리스트에서 해제 하시겠습니까?')) {
			$('input[name="blackListBatchArray"]').val(JSON.stringify(selectedBlackList));

			$('form#blackListBatchCancel').attr('action', '/cms/module/blackList/blackListBatchCancel.do');
			if (doAjaxPost($('form#blackListBatchCancel'))) {
				location.reload();
			}
		}
	});
...
<form:form id="blackListBatchCancel" modelAttribute="blackList" action="blackListBatchCancel.do">
	<form:hidden path="blackListBatchArray"/>
</form:form>

<form:form id="blackListBatch" modelAttribute="blackList" action="blackListBatchAdd.do">
	<form:hidden path="blackListBatchArray"/>
</form:form>

<form:form id="delForm" modelAttribute="student2" action="save.do">
...
<form:form id="studentListForm" modelAttribute="student2" action="student.do">
	<form:hidden path="homepage_id"/>
	<form:hidden path="group_idx"/>
	<form:hidden path="category_idx"/>
	<form:hidden path="large_category_idx"/>
	...
	<div class="infodesk">
		...
		<div class="button btn-group inline">
			<c:if test="${authC}">
				...
				<a href="#" id="excelUpload" class="btn btn2" style="margin-right: 5px;"><i class="fa fa-file-excel-o"></i><span>엑셀등록</span></a>
				<a href="#" id="excelDownloadSample" class="btn btn2" style="margin-right: 5px;"><i class="fa fa-file-excel-o"></i><span>신청서</span></a>
				<a href="#" class="btn btn1 formPrint-btn" style="margin-right: 5px;"><i class="fa fa-print"></i><span>신청서 인쇄</span></a>
				...
				<a href="#" class="btn btn6 left" id="batch-black-add" style="margin-right: 5px;"><i class="fa fa-plus"></i><span>일괄 블랙리스트 등록</span></a>
				<a href="#" class="btn btn left" id="batch-black-cancel" style="margin-right: 5px;"><i class="fa fa-plus"></i><span>일괄 블랙리스트 해제</span></a>
			</c:if>
		</div>
	</div>
	<table class="type1 center">
		<colgroup>
			...
			<col width="15%" />
			<c:choose>
				<c:when test="${trainingInfo.training_status eq '1'}">
					<col width="6%" />
				</c:when>
				<c:otherwise>
					<col width="6%" />
		...
		<thead>
			<tr>
				...
				<th>휴대전화번호<br/>(신청자)</th>
				<th>상태</th>
				<c:if test="${trainingInfo.training_status ne '1'}">
				<th>취소자ID</th>
				<th>취소일</th>
				</c:if>
				<th>기능</th>
...
>데이터가 존재하지 않습니다.</td>
		...
		<c:forEach var="i" varStatus="status" items="${studentList}">
			<tr>
				<td><form:checkbox path="student_idx_arr" cssClass="student_idx_arr" value="${i.student_idx}" data-member-key="${i.member_key}" data-member-name="${i.student_name}"/></td>
				...
				<td>
					...
					<c:if test="${i.student_status eq '1' }">
						<a href="" class="btn dialog-certificate" keyValue="${i.student_idx}">이수증 출력</a>
					</c:if>
				</td>
			</tr>
		</c:forEach>
...
<div id="dialog-2" class="dialog-common" title="이수증">
</div>
...
<div id="dialog-4" class="dialog-common auto-scroll" title="수강생 리스트">
	...
	<input type="hidden" name="training_idx" value="${student.training_idx}"/>

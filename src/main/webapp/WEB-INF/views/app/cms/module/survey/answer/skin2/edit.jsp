...
<link href="/resources/cms/survey/css/skin2.css" rel="stylesheet" type="text/css">
...
<%-- 		<p class="contact"><img src="/resources/cms/survey/img/ico_question.gif" alt="문의" /> <strong>조사문의:</strong>  ${survey.add_user_name} ( ${survey.add_user_tel} )</p> --%>
	</div>
</div>
<!--// 상단 -->
			...
			<tr>
				...
				<form:hidden path="answer_list[${questIdx}].quest_idx" value="${i.quest_idx}" disabled="${i.branch > 0 ? true : false}"/>
					...
					<c:if test="${i.quest_detail_free_yn eq 'Y'}">
						<li>${j.branch_idx}
							${status2.count}) <form:radiobutton id="questIdx_${questIdx}_99" path="answer_list[${questIdx}].quest_idx_list" value="99" disabled="${i.branch > 0 ? true : false}"/>
		...
		<c:when test="${i.quest_type eq 'MULTI'}">
			...
			<tr>
				...
				<form:hidden path="answer_list[${questIdx}].quest_idx" value="${i.quest_idx}" disabled="${i.branch > 0 ? true : false}"/>
					...
					<c:forEach var="j" varStatus="status2" items="${i.quest_detail_list}">
						<li>
							${status2.count}) <form:checkbox id="questIdx_${questIdx}_${status2.count}" path="answer_list[${questIdx}].quest_idx_list[${status2.index}]" value="${status2.count}" branchIdx="${j.branch_idx}" disabled="${i.branch > 0 ? true : false}" />
							<label for="questIdx_${questIdx}_${status2.count}">${j.quest_detail_title}22222222222222</label>
						</li>
					</c:forEach>
					<c:if test="${i.quest_detail_free_yn eq 'Y'}">
						<li>
							${status2.count}) <form:checkbox id="questIdx_${questIdx}_99" path="answer_list[${questIdx}].quest_idx_list" value="99" disabled="${i.branch > 0 ? true : false}"/>
							<label for="questIdx_${questIdx}_99">기타</label>
							<form:input path="answer_list[${questIdx}].short_answer" size="25" maxlength="20" disabled="${i.branch > 0 ? true : false}"/>
						</li>
					</c:if>
			...
			<tr>
				...
				<form:hidden path="answer_list[${questIdx}].quest_idx" value="${i.quest_idx}" disabled="${i.branch > 0 ? true : false}"/>
					<table class="in_tbl" summary="매트릭스형의 세부질문과 보기 내용을 확인할 수 있습니다.">
						<caption style="dispaly:none;">매트릭스형 세부질문 및 보기</caption>
						<colgroup>
							<col width="30" />
							<col />
							<col />
...
<!-- 버튼 -->
<div class="brdBtn">
	<a href="#" class="button save">설문조사 완료</a>
</div>
<!--// 버튼 -->
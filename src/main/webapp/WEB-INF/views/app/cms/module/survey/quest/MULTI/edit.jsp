...
<c:if test="${quest.editMode eq 'modify'}">
	$('span#questDetail').load('/cms/survey/quest/questDetail.do?editMode=modify&quest_type=MULTI&quest_idx=${quest.quest_idx}&homepage_id=${param.homepage_id}&survey_idx=${quest.survey_idx}');
</c:if>
...
$("#dialog-4").dialog({ //개별 모달창 띄울 시 선택자 선언 및 크기 값 설정
	...
	$('select#quest_count').on('change', function(e) {
		$('span#questDetail').load('/cms/survey/quest/questDetail.do?quest_type=MULTI&survey_idx=${quest.survey_idx}&homepage_id=${param.homepage_id}&quest_count=' + $('select#quest_count').val());
	});

...
<script type="text/javascript">
	...
	// 	});
		...
		$('a.goDetail').on('click', function(e) {
			...
			$('input#regNo').val($(this).attr('keyValue'));
			var formData = serializeParameter(['vLoca', 'vCtrl', 'vImg', 'isbn', 'tid', 'menu_idx', 'regNo']);
...
/*
		$('#librarySearch input#search_text').on('keypress', function(e) {
			$('#do-search').click();
		});
*/

	$('#librarySearch input#search_text').on('keyup', function(e) {
		...
		$('a#goNotReqHope').on('click',function(e){
			...
			else if(libName == 'geic' || libName == 'sj' || libName == 'ad')
			{
			...
			else if(libName == 'adys')
			{
				...
				alert(comments);
				location.href="/intro/${context_path}/search/hope/req.do";
			}
			return;
		});
	});
...
<!-- 			<a id="toggleLibList"><span class="orange"><i class="fa fa-building" aria-hidden="true"></i>타도서관 검색</span></a> -->
			<a href="#" id="toggleDetailSearch" class="btn btn1" title="상세검색" tabindex="0" style="border-radius: 15px;float: right; margin-top:-37px; margin-bottom: 16px;" keyValue="${totalSearch.search_type eq 'DETAIL' ? 'show' : 'hide'}"><i class="fa fa-search"></i> 상세 검색</a>
		</div>
		<div class="search-form">			
			<div class="box">			
				<div class="box1">
					<select id="search_type" name="search_type" class="selectmenu" style="width:100%;padding-left:12px;border:0px;">
			...
			<!--<div id="autoFill"> -->
			<!--</div> -->
			<div class="srch-btn-box">
				<a id="do-search" class="btnNew">검색하기</a>
		...
		<div class="smain">
			<div class="box"<c:if test="${!showSmain}">style="display: none;"</c:if>>
				...
				<div id="search-results" class="search-results">				
					<c:forEach items="${result.dsResult}" var="i">
						<div class="row">
							<p class="admin"><input name="print_param" type="checkbox" class="checkBook" /></p>
							<div class="thumb">
								<c:choose>
									<c:when test="${i.IMAGE_URL eq '' or fn:contains(i.IMAGE_URL, 'noimg')}">
										<a href="#" vLoca="${fn:escapeXml(i.LIMT06)}" vCtrl="${fn:escapeXml(i.CTRL)}" vImg="${fn:escapeXml(i.IMAGE_URL)}" isbn="${fn:escapeXml(i.DISP08)}" tid="${fn:escapeXml(i.tid)}" keyValue="${fn:escapeXml(i.LIMT06)}_${fn:escapeXml(i.CTRL)}_${fn:escapeXml(i.tid)}_${fn:escapeXml(i.IMAGE_URL)}"  class="goDetail noImg">
											<img src="/resources/common/img/noImg.gif" alt="noImage"/>
										</a>
									</c:when>
									<c:otherwise>
										<a href="#" vLoca="${fn:escapeXml(i.LIMT06)}" vCtrl="${fn:escapeXml(i.CTRL)}" vImg="${fn:escapeXml(i.IMAGE_URL)}" isbn="${fn:escapeXml(i.DISP08)}" tid="${fn:escapeXml(i.tid)}" keyValue="${fn:escapeXml(i.LIMT06)}_${fn:escapeXml(i.CTRL)}_${fn:escapeXml(i.tid)}_${fn:escapeXml(i.IMAGE_URL)}"  class="goDetail">
							...
							<div class="box">
								<div class="item">
									<div class="bif">
										...
										<a href="#" vLoca="${fn:escapeXml(i.LIMT06)}" vCtrl="${fn:escapeXml(i.CTRL)}" vImg="${fn:escapeXml(i.IMAGE_URL)}" isbn="${fn:escapeXml(i.DISP08)}" tid="${fn:escapeXml(i.tid)}" keyValue="${fn:escapeXml(i.LIMT06)}_${fn:escapeXml(i.CTRL)}_${fn:escapeXml(i.tid)}_${fn:escapeXml(i.IMAGE_URL)}"  class="name goDetail">${fn:replace(fn:escapeXml(i.DISP01), fn:escapeXml(librarySearch.search_text), replaceStr)}</a>
			...
			<%--<div class="ws-filter" id="hotTrend" style="height: 370px; <c:if test="${fn:length(result.dsResult) < 1 and !showSmain}">clear: both;</c:if>"> --%>
				<!--<h4>실시간 검색어 순위</h4> -->
					<!--<div style="text-align: center; padding-top: 105px;" > -->
					<!--불러오는 중... -->
					<!--</div> -->
			<!--</div> -->
			<div class="ws-filter" <c:if test="${fn:length(result.dsResult) < 1 or !showSmain}"> style="display: none;"</c:if>>
				<h4>검색결과 제한</h4>
				<ul>

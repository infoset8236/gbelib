...
$(function(){
	...
	$('a#apply').on('click', function(event) {		
		doGetLoad('/intro/${context_path}/module/facility/edit.do', 'editMode=ADD&facility_idx='+$(this).attr('keyValue')+'&menu_idx='+$('input#menu_idx').val());
...
<form:hidden id="homepage_id_1" path="homepage_id"/>
	...
	<div class="ym_btns">
		<a id="before-btn" href="#prev" class="btn prev"><i class="fa fa-angle-left" style="width:30px;height:40px;"></i><span class="blind">이전달</span></a>
		&nbsp;&nbsp;&nbsp;
		<form:select path="plan_year" class="" style="width:150px;height:50px;"></form:select>
		&nbsp;&nbsp;&nbsp;
		<form:select path="plan_month" class="" style="width:120px;height:50px;"></form:select>
		&nbsp;&nbsp;&nbsp;
		<a id="next-btn" href="#next" class="btn next"><i class="fa fa-angle-right" style="width:30px;height:40px;"></i><span class="blind">다음달</span></a>
	</div>
	<div id="calendar">
		<div class="cal-func">
			<div class="date-view">		
				...
				<tr class="week">
					...
					<c:otherwise>
						<td class="sun">
							...
							<ul>
								<tag:facility plan_date="${plan_date}" facilityList="${facilityList}" calendarManageList="${calendarManageList}" facilityReqList="${facilityReqList}" countApply="${countApply}" mode="admin" />
							</ul>												
						</td>
					</c:otherwise>
					</c:choose>

...
$(function() {
	$('.view-detail').on('click', function(e) {
		...
		location.href = 'detail.do?vSeqNo=' + seq_no + '&menu_idx=${librarySearch.menu_idx}';
	});
	
	$('a.excel-btn').on('click', function(e) {
		e.preventDefault();
		$('#excelDownForm #excel_type').val('BEACH');
...
<div class="book-list" style="padding-bottom:60px;">
	<c:if test="${fn:length(nightResveList.dsMyLibraryList) < 1 }"> <h3>야간예약중인 도서 내역이 없습니다.</h3></c:if>
	<c:if test="${fn:length(nightResveList.dsMyLibraryList) > 0 }">
	<div class="alarm_list">
		<ul>
		<c:forEach items="${nightResveList.dsMyLibraryList}" var="i" varStatus="status">
		<li class="view-detail" data-seq_no="${i.SEQ_NO}">
...
<div class="book-list">
	...
	<c:if test="${fn:length(nightResveList.dsMyLibraryList) > 0 }">
		<table summary="신청정보">
			...
			<tbody>
				<c:forEach items="${nightResveList.dsMyLibraryList}" var="i" varStatus="status">
					<tr class="view-detail" data-seq_no="${i.SEQ_NO}">

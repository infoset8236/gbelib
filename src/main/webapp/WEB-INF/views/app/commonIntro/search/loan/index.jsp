...
$(function() {
	...
	$('a.excel-btn').on('click', function(e) {
		...
		$('#excelDownForm #excel_type_detail').val('0001');
	...
	$('a.loanRenew').on('click', function(e) {
		e.preventDefault();
		$('#vLoanNo').val($(this).attr('keyValue'));
		$('#editMode').val('RENEW');
	...
	$('#selectAll').click(function (e) {
		...
		$('.returnDelayArray').each(function () {
			$(this).prop('checked', isSelectAllChecked);
		});
	});

	$('.loanCheckbox').on('change', function () {
		var allChecked = $('.returnDelayArray').length === $('.returnDelayArray:checked').length;
		$('#selectAll').prop('checked', allChecked);
	});

	$('#batchReturn').click(function (event) {
		event.preventDefault();

		var selectedLoans = $('.returnDelayArray:checked').map(function () {
			return $(this).val();
		}).get();
		...
		if (confirm('선택된 도서를 대출 연장 하시겠습니까??')) {
			$('form#librarySearch').attr('action', 'bookBatchReturnDelay.do');
...
<c:if test="${fn:length(loanList.dsMyLibraryList) > 0}">
	<div style="text-align: right">
		...
		<c:if test="${homepage.context_path eq 'yc'}">
			<button id="batchReturn" class="btn">일괄 대출 연장</button>
		</c:if>
		<form:form id="excelDownForm" modelAttribute="librarySearch" action="/${homepage.context_path}/intro/search/excelDownload.do" method="get">
			...
			<form:hidden path="excel_type_detail"/>
		</form:form>
	</div>
</c:if>
...
<form:form modelAttribute="librarySearch" action="index.do" >
<c:choose>
...
<div class="mylibrary-btn-section">
	<ul>
		<li><a href="/${homepage.context_path}/intro/search/loan/index.do?menu_idx=5" class="on"><img src="/resources/homepage/app/img/mylib01.png" alt=""><br/>대출중도서</a></li>
...
<div class="book-list" style="padding-bottom:60px;">
	<c:if test="${fn:length(loanList.dsMyLibraryList) < 1 }"> <h4>조회된 도서가 없습니다.</h4></c:if>
	<c:if test="${fn:length(loanList.dsMyLibraryList) > 0 }">
	...
	<c:if test="${fn:length(loanList.dsMyLibraryList) > 0 }">
		<table summary="신청정보">
			...
			<tbody>
				...
				<tr>
					<c:choose>
						<c:when test="${homepage.context_path eq 'yc' && i.RETURN_CHECK eq true}">
							<td>
								<form:checkbox path="returnDelayArray" class="returnDelayArray" value="${i.LOAN_NO}" />
							</td>
						</c:when>
						<c:when test="${homepage.context_path eq 'yc' && i.RETURN_CHECK eq false}">
							<td></td>
						</c:when>
					</c:choose>
					<c:if test="${homepage.context_path eq 'yc' && i.RETURN_CHECK eq true}">
					...
					<td class="view-detail" data-loan_no="${i.LOAN_NO}">${i.RETURN_TYPE_NAME}</td>
				</tr>

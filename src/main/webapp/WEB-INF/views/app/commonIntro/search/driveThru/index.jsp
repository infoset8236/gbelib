...
<script type="text/javascript">
    $(function() {
        $('a.excel-btn').on('click', function(e) {
            e.preventDefault();
            $('#excelDownForm #excel_type').val('DRIVETHRU');
...
<c:choose>
    ...
    <c:otherwise>
        <c:if test="${fn:length(driveThruList.dsMyLibraryList) > 0}">
...
<c:choose>
    <c:when test="${homepage.context_path eq 'app'}">
        ...
        <div class="book-list" style="padding-bottom:60px;">
            <c:if test="${fn:length(driveThruList.dsMyLibraryList) < 1 }"> <h3>당일픽업예약 내역이 없습니다.</h3></c:if>
            <c:if test="${fn:length(driveThruList.dsMyLibraryList) > 0 }">
                <div class="alarm_list">
                    <ul>
                        <c:forEach items="${driveThruList.dsMyLibraryList}" var="i" varStatus="status">
                            <li class="view-detail" data-loca_no="${i.BOOK_LOCA}">
                                <p class="book_title">${status.index+1}. ${i.TITLE} / ${i.BOOK_LOCA_NAME}</p>
                                <p class="book_status">${i.STATUS_NAME}</p>
                                <p class="date">예약일 : <fmt:parseDate var="curDate" value="${i.REQST_DATE}" pattern="yyyyMMdd"/>
        ...
        <div class="book-list">
            ...
            <c:if test="${fn:length(driveThruList.dsMyLibraryList) > 0 }">
                <table summary="신청정보">
                    ...
                    <tbody>
                    <c:forEach items="${driveThruList.dsMyLibraryList}" var="i" varStatus="status">
                        <tr class="view-detail" data-loca_no="${i.BOOK_LOCA}">
                            ...
                            <td>${i.STATUS_NAME}</td>
                            <c:choose>
                                <c:when test="${i.STATUS_CODE eq '0010'}">
                                    <td><a href="" class="btn close-cancel" vSeqNo="${i.SEQ_NO}" vLoca="${i.BOOK_LOCA}" vTitle="${i.TITLE}">신청취소</a></td>
                                </c:when>
                                <c:otherwise>
                                    <td></td>
                                </c:otherwise>

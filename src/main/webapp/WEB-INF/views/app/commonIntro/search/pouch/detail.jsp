...
$(function() {
	$('a.resve-cancel').on('click', function(e) {
		$('#pouchCancelForm #editMode').val('CANCEL');
		$('#pouchCancelForm #vSeqNo').val($(this).attr('vSeqNo'));
		
		if ( doAjaxPost($('#pouchCancelForm')) ) {
			location.reload();
		}
		e.preventDefault();
	});
...
<form:form id="pouchCancelForm" modelAttribute="librarySearch" action="save.do">
	<form:hidden path="editMode" value="CANCEL"/>
	<form:hidden path="vSeqNo"/>
</form:form>
<div class="book-list">
	<c:if test="${empty pouchDetail}"><h3>예약중인 도서 내역이 없습니다.</h3></c:if>
	<c:if test="${not empty pouchDetail}">
		<c:forEach items="${pouchDetail.dsMyLibraryListD}" var="i"> 
			<div class="row">
				<div class="box">
					<div class="item">
						<div class="bif">
							<div class="top">
								...
								<div class="control">
									<a href="" class="btn resve-cancel" vSeqNo="${i.SEQ_NO}">신청취소</a>
								</div>
								</c:if>
							</div>

...
<form:form id="searchTableForm" modelAttribute="librarySearch" method="GET">
	<c:forEach items="${result.dsResult}" var="i">
		<div class="row">
						<p class="admin"><input name="print_param" type="checkbox" class="checkBook" value="${fn:escapeXml(i.LIMT06)}__${fn:escapeXml(i.CTRL)}__ __${fn:escapeXml(i.IMAGE_URL) eq '' ? ' ' : fn:escapeXml(i.IMAGE_URL)}__${fn:escapeXml(i.DISP07)}__${fn:escapeXml(i.DISP04)}__${fn:escapeXml(i.DISP01)}__${fn:escapeXml(i.DISP02)}__${fn:escapeXml(i.DISP03)}__${lib_name}" title="${fn:replace(fn:escapeXml(i.DISP02), fn:escapeXml(librarySearch.search_text), replaceStr)}"/></p>
			<div class="thumb">
					<c:choose>
			...
			<div class="box">
				<div class="item">
					<div class="bif">
						...
						<a href="#" vLoca="${fn:escapeXml(i.LIMT06)}" vCtrl="${fn:escapeXml(i.CTRL)}" vImg="${fn:escapeXml(i.IMAGE_URL)}" isbn="${fn:escapeXml(i.isbn)}" tid="${fn:escapeXml(i.tid)}" class="name goDetail">${fn:replace(fn:escapeXml(i.DISP01), fn:escapeXml(librarySearch.search_text), replaceStr)}</a>

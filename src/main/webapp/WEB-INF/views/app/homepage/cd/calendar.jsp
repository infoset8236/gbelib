			...
			<div class="inbox">
				<ul>
					<c:forEach var="i" begin="1" end="31" varStatus="status">
						<c:set var="key" value="${i < 10 ? '0':''}${i}"/>
						<c:set var="idx" value="${''}${i}"/>
						<c:forEach items="${calendarResult[idx][0]}" var="j">
							<li class="planList">
								<div class="outer">
									<div class="inner">
										<c:choose>
											<c:when test="${fn:contains(j, '휴관')}">
												<div class="schedule-title hol">${fn:replace(fn:substringBefore(j, ']'), '[', '')}</div>
											...
											<c:otherwise>
												<div class="schedule-title cul">${fn:replace(fn:substringBefore(j, ']'), '[', '')}</div>
												<div class="schedule-contents">
													<p class="datetime">${calendar.plan_date}-${key}</p>
													<p class="title">
														<em>

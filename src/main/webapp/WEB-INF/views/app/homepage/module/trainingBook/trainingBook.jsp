...
$(function(){
	...
	$('a.print-btn').on('click', function(e) {
		...
		divToPrint.prepend('<h1>${trainingBook.sel_date} [${training.group_name}]${training.category_name} : ${training.training_name}</h1>');
	...
	$('a#month_prev').on('click', function(e) {
		...
		$('div#trainingBookLayer').load('trainingBook.do?'+serializeCustom($('form#trainingBook')));
	});
	...
	$('a.status-btn').on('click', function(e) {
		...
		var scrollLeftValue = $('div#trainingBookScroll').scrollLeft();
		if ( doAjaxPost($('#saveForm')) ) {
			$('div#trainingBookLayer').load('trainingBook.do?'+serializeCustom($('#saveForm')), function() {
				$('div#trainingBookScroll').scrollLeft(scrollLeftValue);
			});
		}
	});
	...
	//$('#trainingBookArea thead th, #trainingBookArea thead td').css('height', '20px');
...
<div class="wrapper wrapper-white " >
	<form:form modelAttribute="trainingBook" action="index.do" method="get">
		<form:hidden path="menu_idx"/>
		<form:hidden path="homepage_id"/>
		<form:hidden path="group_idx"/>
		<form:hidden path="category_idx"/>
		...
		<div class="calendar" style="width: 100%;">
			...
			<div class="infodesk center">
				...
				<div class="radio" style="margin-left: 15px;">
					<input type="radio" name="status" id="radio13" value="0" <c:if test="${trainingBook.status eq 0}">checked="checked"</c:if>/>

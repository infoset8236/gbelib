...
$(function(){
	...
	$('a.cancel').on('click', function(e) {
		...
		if (confirm("해당 강좌 신청을 취소하시겠습니까? 취소후 해당 강좌에 대해 재신청 가능합니다.")) {
			...
			$('input#training_idx').val($(this).attr('keyValue4'));
	...
	$('a.dialog-certificate').on('click', function(e) { 
		$('#dialog-1').load('student/certificate.do?homepage_id=' + $(this).attr('keyValue1') + '&group_idx=' + $(this).attr('keyValue2') + '&category_idx=' + $(this).attr('keyValue3') + '&training_idx=' + $(this).attr('keyValue4') + '&student_idx=' + $(this).attr('keyValue5'), function( response, status, xhr ) {
	...
	$('a#search-btn').on('click', function(e) {
		e.preventDefault();
		doGetLoad('applyList.do', $('form#training').serialize());
	});
});
</script>
...
<c:if test="${fn:length(trainingList) > 0}">
	<div style="text-align: right">
		<form:form id="excelDownForm" modelAttribute="training" action="/${homepage.context_path}/module/training/excelDownload.do" method="get">
...
<form:form modelAttribute="training" action="/${homepage.context_path}/module/training/student2/save.do" method="POST">
	<form:hidden path="homepage_id" />
	<form:hidden path="group_idx"/>
	<form:hidden path="category_idx"/>
	<form:hidden path="training_idx"/>
		...
		<c:forEach items="${trainingList}" var="i">
			<div class="item">
				<div class="op_title category">
					...
					<a href="" class="name detail-btn" keyValue1="${i.group_idx}" keyValue2="${i.category_idx}" keyValue3="${i.training_idx}">${i.training_name}</a>
				</div>
				<div class="box">
					<div class="box2">
						<ul class="con2">
							...
							<li><div class="status">
								...
								<span>
									온라인 :
									<span ${i.training_join_count > 0 and (i.training_join_count eq i.training_limit_count)? 'style="color:red;"' : 'style="color:orange"'}>${i.training_join_count}</span> / ${i.training_limit_count}
								</span>
								<c:if test="${i.training_offline_count > 0}">
									<span>
										오프라인 :
										<span ${i.training_off_join_count > 0 and (i.training_off_join_count eq i.training_offline_count)? 'style="color:red;"' : 'style="color:orange"'}>${i.training_off_join_count}</span> / ${i.training_offline_count}
									</span>
								</c:if>
								<c:if test="${i.training_backup_count > 0}">
									<span>
										(
										후보자 :
										<span ${i.training_backup_join_count > 0 and (i.training_backup_join_count eq i.training_backup_count)? 'style="color:red;"' : 'style="color:orange"'}>${i.training_backup_join_count}</span> / ${i.training_backup_count}
										)
									</span>
								</c:if>
							</div></li>
							...
							<li>
								<div>
									...
									<c:choose>
										<c:when test="${i.training_status eq '3'and i.wait_num != 0}">
											<span>현재 대기번호 ${i.wait_num}번 입니다.</span>
										</c:when>
										<c:when test="${i.training_status eq '4'and i.wait_num != 0}">
											<span>현재 대기번호 ${i.wait_num}번 입니다.</span>
										</c:when>
									</c:choose>
								</div>
							...
							<li>
								<div>
									<label>기관명</label> : ${i.belong_name}
								</div>
							</li>
							<li>
								<div>
									<label>신청자 주소</label> : ${sessionScope.member.address1}
								</div>
							</li>
							<li>
								<div>
									<label>신청자 전화번호</label> : ${sessionScope.member.mobile_no}
								</div>
							</li>
						</ul>
					</div>
...
<!-- 							<i class="fa fa-circle-o"></i><span>신청완료</span></a> -->
							<a href="" class="btn btn5 cancel" keyValue1="${i.homepage_id}" keyValue2="${i.group_idx}" keyValue3="${i.category_idx}" keyValue4="${i.training_idx}">
...
<!-- 							<i class="fa fa-sign-in"></i><span>대기자</span></a> -->
						...
						<c:when test="${i.training_status eq '4'}">
						...
						<c:when test="${i.training_status eq '7' }">
							<a href="javascript:void(0);" class="btn btn3" style="cursor: default;">
							<i class="fa fa-times-circle"></i><span>취소완료</span></a>
						</c:when>
						<c:when test="${i.training_status eq '8' }">
							<a href="javascript:void(0);" class="btn btn3" style="cursor: default;">
							<i class="fa fa-times-circle"></i><span>관리자취소</span></a>
						</c:when>
						<c:when test="${i.training_status eq '10'}">

...
$(function(){
	$('a.detail-btn').on('click', function(e) {
		...
		param += '&training_idx='+$(this).attr('keyValue3');
		param += '&menu_idx='+$('input#menu_idx').val();
		
		doGetLoad('/${homepage.context_path}/module/training/detail.do', param);
		e.preventDefault();
	});
	
	$('a.add').on('click', function(e) {
		var $this = $(this);
		doGetLoad('/${homepage.context_path}/module/training/student/edit.do', 
				'editMode=ADD&homepage_id='+$this.attr('keyValue1')+'&group_idx='+$this.attr('keyValue2')+'&category_idx='+$this.attr('keyValue3')
				+'&training_idx='+$this.attr('keyValue4')+ '&apply_status='+ $this.attr('apply_status')+'&menu_idx='+$('input#menu_idx').val());
	...
	$('a.trainingBook-btn').on('click', function(e) {
		e.preventDefault();
		var $this = $(this);
		doGetLoad('/${homepage.context_path}/module/trainingBook/index.do','menu_idx='+$('#menu_idx').val()+'&homepage_id='+$this.attr('keyValue1')+'&group_idx='+$this.attr('keyValue2')+'&category_idx='+$this.attr('keyValue3')+'&training_idx='+$this.attr('keyValue4'))
	});
	
	$('a#search-btn').on('click', function(e) {
		e.preventDefault();
		doGetLoad('index.do', $('form#training').serialize());
	});
...
/* 		$('select#category_idx').select2({minimumResultsForSearch: Infinity}); */
	...
	$('div#board_paging a').on('click', function(e) {
		...
		var param = serializeCustom($('form#training'));
		...
		<ul>
			...
			<li style="padding-bottom: 5px;">
				...
				<form:select path="large_category_idx" cssClass="selectmenu" cssStyle="width: 200px;" title="대분류선택">
					...
					<c:forEach var="i" varStatus="status" items="${trainingLargeCategoryList}">
					<form:option value="${i.training_code}" label="${i.code_name}" />
					</c:forEach>
				</form:select>
			...
			<li style="padding-bottom: 5px;">
				연령구분 : <form:checkboxes items="${trainingAgeDivCodeList}" path="program_age_div_arr" itemLabel="code_name" itemValue="training_code" cssStyle="margin-left:10px;"/> 
			</li>
			<li style="padding-bottom: 5px;">
...
<div class="op_wrap">
	<div class="smain">
		<c:forEach items="${trainingList}" var="i">
			<div class="item" id="${i.homepage_id}_${i.group_idx}_${i.category_idx}_${i.training_idx}">
				<div class="op_title category">
					...
					<a href="#" class="name detail-btn" keyValue="${i.homepage_id}" keyValue1="${i.group_idx}" keyValue2="${i.category_idx}" keyValue3="${i.training_idx}">${i.training_name}</a>
				</div>
				<div class="box">
					<div class="box2">
						<ul class="con2">
				...
				<div class="stat">
					<c:choose>
						<c:when test="${member.login and (member.loginType eq 'HOMEPAGE') and (i.member_key eq member.seq_no)}">
							<a class="btn btn3 trainingBook-btn" keyValue1="${i.homepage_id}" keyValue2="${i.group_idx}" keyValue3="${i.category_idx}" keyValue4="${i.training_idx}">출석부</a>
						</c:when>
						<c:otherwise>
							<c:choose>
								<c:when test="${i.training_status eq '0'}">
									<a href="#" class="btn btn1 add" keyValue1="${i.homepage_id}" keyValue2="${i.group_idx}" keyValue3="${i.category_idx}" keyValue4="${i.training_idx}" apply_status="1">
									<i class="fa fa-pencil-square-o"></i><span>수강신청 </span></a>
								</c:when>
								<c:when test="${i.training_status eq '1'}">
									<a href="#" class="btn btn1 add" keyValue1="${i.homepage_id}" keyValue2="${i.group_idx}" keyValue3="${i.category_idx}" keyValue4="${i.training_idx}" apply_status="2">

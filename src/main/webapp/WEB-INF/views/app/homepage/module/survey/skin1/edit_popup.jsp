...
<link href="/resources/cms/survey/css/type1.css" rel="stylesheet" type="text/css">
...
// 		doAjaxPost($form);
	...
	<c:if test="${!(survey.survey_open_yn eq 'Y' and !already)}">
...
<style>
caption {position:absolute;top:0;left:0;width:0;height:0;overflow:hidden;font-size:0;line-height:0;text-indent:-999px}
</style>
...
<div class="brdBtn">
	<c:if test="${quest.open_yn eq 'Y' and quest.survey_open_yn eq 'N'}">
		<c:if test="${homepage.homepage_id ne 'h24'}">
			<a href="" class="button statistics">
				<i class="fa fa-bar-chart"></i><span>통계보기</span>
			</a>
		</c:if>
	</c:if>
	<c:if test="${survey.survey_open_yn eq 'Y' and !already}">
	<a href="" class="button save">
		<i class="fa fa-pencil-square-o"></i><span>설문완료</span>
	</a>
	</c:if>

	<c:if test="${quest.popup_yn eq 'Y' }">
		<a href="#" class="button close">닫기</a>
	</c:if>
	<c:if test="${quest.popup_yn eq 'N' }">
		<a href="" class="btn btn1 list">
			<span>목록으로</span>
		</a>
	</c:if>
</div>
...
<div class="info">
	...
	<p class="t2">
		...
		<fmt:formatDate value="${now}" pattern="d" var="today"/>
		${today}
	</p>
	<ul>
		<!-- <li>
...
<div class="cal_area">
	<div id="calendar">
		<div class="cal-func">
			<a id="before-btn" href="#prev" class="btn prev" keyValue="${calendar.plan_date}"><i class="fa fa-angle-left"></i><span class="blind">이전달</span></a> <b class="date"><span>${fn:split(calendar.plan_date, '-')[0]}/</span><em>${fn:split(calendar.plan_date, '-')[1]}</em></b> <a id="next-btn" href="#next" class="btn next" keyValue="${calendar.plan_date}"><i class="fa fa-angle-right"></i><span class="blind">다음달</span></a>
		</div>
									...
									<c:otherwise>
										...
										<c:choose>
											<c:when test="${fn:indexOf(closeDayList.dd, one) > -1 }">
												<a class="type-m" keyValue="${i.sun}">${i.sun}</a>
											</c:when>
											<c:otherwise>
												<a class="type-r" keyValue="${i.sun}">${i.sun}</a>
											</c:otherwise>
										</c:choose>
									</c:otherwise>
								</c:choose>
							...
							<div>
								<c:choose>
									...
									<c:otherwise>
										...
										<c:choose>
											<c:when test="${fn:indexOf(closeDayList.dd, one) > -1 }">
												<a id="showCal" class="type-m" keyValue="${i.mon}">${i.mon}</a>
											</c:when>
											<c:otherwise>
							...
							<div>
								<c:choose>
									...
									<c:otherwise>
										...
										<c:choose>
											<c:when test="${fn:indexOf(closeDayList.dd, one) > -1 }">
												<a id="showCal" class="type-m" keyValue="${i.tue}">${i.tue}</a>
											</c:when>
											<c:otherwise>
							...
							<div>
								<c:choose>
									...
									<c:otherwise>
										...
										<c:choose>
											<c:when test="${fn:indexOf(closeDayList.dd, one) > -1 }">
												<a id="showCal" class="type-m" keyValue="${i.wed}">${i.wed}</a>
											</c:when>
											<c:otherwise>
							...
							<div>
								<c:choose>
									...
									<c:otherwise>
										...
										<c:choose>
											<c:when test="${fn:indexOf(closeDayList.dd, one) > -1 }">
												<a id="showCal" class="type-m" keyValue="${i.thu}">${i.thu}</a>
											</c:when>
											<c:otherwise>
							...
							<div>
								<c:choose>
									...
									<c:otherwise>
										...
										<c:choose>
											<c:when test="${fn:indexOf(closeDayList.dd, one) > -1 }">
												<a id="showCal" class="type-m" keyValue="${i.fri}">${i.fri}</a>
											</c:when>
											<c:otherwise>
							...
							<div>
								<c:choose>
									...
									<c:otherwise>
										...
										<c:choose>
											<c:when test="${fn:indexOf(closeDayList.dd, one) > -1 }">
												<a id="showCal" class="type-m" keyValue="${i.sat}">${i.sat}</a>
											</c:when>
											<c:otherwise>
		...
		<div class="inbox" id="popup_layer" style="display: none;">
				<c:forEach var="i" items="${calendarResult}" varStatus="status">
					<div id="${i.key}" class="calAll center" style="display: none;">
						<dl>
